...
But we're millenials.  We want multi-colored disco lights and an alphabet of characters that only <a href="https://mrrobot.fandom.com/wiki/Whiterose">White Rose</a> and the Dark Army might understand.
...
Embedding the `<iframe>` into this page (so that one can peek at the monochrome green showers of the Tildeverse in real-time), is a bit more complicated.  Using an intermediate proxy deployed to a <a href="https://workers.cloudflare.com/">Cloudflare worker</a>, you can bypass any <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS">CORS</a> requirements set by the Tildeverse origin site and convince your browser that it's OK to be susceptible to <a href="https://www.w3.org/Security/wiki/Comparison_of_CORS_and_UMP#CORS">clickjacking</a>. But don't actually clickjack, it's not nice.
